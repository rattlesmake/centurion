...
{
	this->cells = std::vector<Cell>{(uint64_t)this->height * this->width, Cell(_height, _width)};
}
#pragma endregion
...
#if GOAL_BOUNDING
void PreComputedMap::ComputeGoalBounding(void)
{
	std::shared_ptr<DijkstraFloodfill> dijkstra = DijkstraFloodfill::CreateDijkstraFloodfill(this);

	for (int startRow = 0; startRow < this->height; startRow++)
	{
		for (int startCol = 0; startCol < this->width; startCol++)
		{
			if (this->IsWall(startRow, startCol) == true)
				continue;

			dijkstra->Flood(startRow, startCol);
			const uint32_t currentIteration = dijkstra->GetCurrentInteration();
			...
			for (int r = 0; r < this->height; r++)
			{
				for (int c = 0; c < this->width; c++)
				{
					...
					const DijkstraPathfindingNode* const node = dijkstra->GetDijkstraNode(r,c);
					const uint8_t dir = node->directionFromStart;

					if (node->iterations == currentIteration && node->listInWhichIAm == PathfindingNodeStatus::OnClosedList && dir >= 0 && dir <= 7)
					{
						const int row = node->row;
						const int col = node->col;
						const uint32_t index = startCol + (startRow * this->width);
						
						if (this->cells[index].GetGoalBound(dir, Cell::GoalBoundsEnum::MinRow) > row)
						{
							this->cells[index].SetGoalBound(dir, Cell::GoalBoundsEnum::MinRow, row);
						}
						if (this->cells[index].GetGoalBound(dir, Cell::GoalBoundsEnum::MaxRow) < row)
						{
							this->cells[index].SetGoalBound(dir, Cell::GoalBoundsEnum::MaxCol, row);
						}
						if (this->cells[index].GetGoalBound(dir, Cell::GoalBoundsEnum::MinCol) > col)
						{
							this->cells[index].SetGoalBound(dir, Cell::GoalBoundsEnum::MinCol, col);
						}
						if (this->cells[index].GetGoalBound(dir, Cell::GoalBoundsEnum::MaxCol) < col)
						{
							this->cells[index].SetGoalBound(dir, Cell::GoalBoundsEnum::MaxCol, col);
						}
					}
				}
			}
