...
#include <GL/glew.h>
#include <png.h> // rattlesmake library
...
namespace ifd {
	...
	FileDialog::FileDialog() {
		...
		this->CreateTexture = [](uint8_t* data, int w, int h, char fmt) -> void* {
			GLuint tex;
	...
	void FileDialog::m_clearIconPreview()
	{
		...
		for (auto& data : m_content) {
			...
			if (data.IconPreviewData != nullptr) {
				rattlesmake::image::stb::free(data.IconPreviewData);
		...
		for (size_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++) {
			...
			if (data.Path.has_extension()) {
				...
				if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".tga") {
					...
					unsigned char* image = rattlesmake::image::stb::load_from_file(data.Path.string().c_str(), &width, &height, &nrChannels, (int)rattlesmake::image::png_type::RGBA);
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				if (entry.HasIconPreview && entry.IconPreviewData != nullptr) {
					...
					rattlesmake::image::stb::free(entry.IconPreviewData);
