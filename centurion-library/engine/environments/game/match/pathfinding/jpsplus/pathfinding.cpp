...
namespace path_finding
{
	...
	{
		//---------------------- TEMP ----------------------
		return std::list<glm::ivec2>{pStart, pDest};
		//--------------------------------------------------
	...
	void Pathfinding::FinalizePath(std::list<glm::ivec2>& finalPath)
	{
		...
		PathfindingNode* curNode = this->destNode;
		...
		while (curNode != nullptr)
		{
			...
			if (prevNode != nullptr)
			{
				...
				for (int i = 0; i < steps; i++)
				{
					...
					finalPath.push_front(locNew);
				}
			}
			
			finalPath.push_front(loc);
			prevNode = curNode;
			curNode = curNode->parentNodePtr;
		}
		this->Smooth(finalPath);
	}
	...
	void Pathfinding::Smooth(std::list<glm::ivec2>& finalPath) const
	{
		std::list<glm::ivec2>::iterator checkPoint = finalPath.begin();
		std::list<glm::ivec2>::iterator currentPoint = std::next(checkPoint, 1);
		while (std::next(currentPoint, 1) != finalPath.end())
		{
			if (this->IsWalkable(checkPoint, std::next(currentPoint, 1)) == true)
			{
				currentPoint = finalPath.erase(currentPoint);
			}
			else
			{
				checkPoint = currentPoint;
				currentPoint = std::next(currentPoint, 1);
			}
		}
	}

	inline bool Pathfinding::IsWalkable(const std::list<glm::ivec2>::iterator& pointA, const std::list<glm::ivec2>::iterator& pointB) const
	{
		...
		for (int i = 0; i < steps; i++)
		{
			x += xInc;
			y += yInc;
			//Da mettere dentro pcm non verificando le celle qualora esse escano dalla mappa.
			if (this->pcm.IsEmpty(y + 1, x) == false
				|| this->pcm.IsEmpty(y - 1, x) == false
				|| this->pcm.IsEmpty(y, x + 1) == false
				|| this->pcm.IsEmpty(y, x - 1) == false)
				return false;
		}
		return true;
	...
	namespace
	{
		Pathfinding* instance = nullptr;
	}

	bool Init(const int mapHeight, const int mapWidth)
	{
		if (path_finding::instance == nullptr)
		{
			path_finding::instance = new Pathfinding(mapHeight, mapWidth);
			return true;
		}
		return false;
	}
	...
	bool Reset(void)
	{
		if (path_finding::instance != nullptr)
		{
			delete path_finding::instance;
			path_finding::instance = nullptr;
			return true;
		}
		return false;
	}
