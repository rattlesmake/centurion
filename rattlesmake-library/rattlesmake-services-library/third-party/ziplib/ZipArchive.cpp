...
std::vector<std::string> ZipArchive::GetEntriesInInternalFolder(const std::string& folderName, const std::string& extension, const EntryType requestType)
{
    ...
    for (auto const& entry : _entries)
    {
        ...
        if (requestType == EntryType_File && entry->IsDirectory()) continue;
        if (requestType == EntryType_Folder && entry->IsDirectory() == false) continue;

        if (fullName.find(folderName) != std::string::npos)
        {
            if (extension.empty() || type == "*") 
            {
                output.push_back(entry->GetFullName());
            }
            else
            {
